...
$files = Get-ChildItem $releaseDir -File |
    Where-Object {
        ...
        $_.Name -like "*Setup-$version.exe" -or
        $_.Name -like "*Setup-$version.exe.blockmap"
    }

if (-not $files) {
    Write-Error "No release files found for version $version. Run: npm run release:github"
}
